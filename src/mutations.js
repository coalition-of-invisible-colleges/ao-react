...
function tasksMuts(tasks, ev) {
    switch (ev.type) {
        ...
        case "ao-outbound-connected":
            tasks.push(
                calculations.blankCard(ev.address, ev.address, "purple")
            );
            break;
        case "ao-disconnected":
            break;
        case "resource-created":
            tasks.push(
                calculations.blankCard(ev.resourceId, ev.resourceId, "red")
            );
            break;
        case "member-created":
            tasks.push(
                calculations.blankCard(ev.memberId, ev.memberId, "blue")
            );
            break;
        case "task-created":
            tasks.push(
                calculations.blankCard(ev.taskId, ev.name, ev.color, ev.deck)
            );
            tasks.forEach(task => {
                ...
                if (task.taskId === ev.taskId) {
                    ...
                    do {
                        ...
                        crawler = crawler.forEach(t => {
                            ...
                            if (subTask.length > 1) {
                                console.log(
                                    "duplicate task found, this is very bad"
                                );
                            }
                            subTask = subTask[0];
                            if (
                                ...
                                console.log(
                                    "invalid task data found, this is very bad"
                                );
                                return;
                            }
        ...
        case "task-unclaimed":
            tasks.forEach(task => {
                if (task.taskId === ev.taskId) {
                    task.claimed = task.claimed.filter(
                        mId => mId !== ev.memberId
                    );
                    if (task.claimed.length < 1) {
                        tasks.forEach(p => {
                            if (
                                ...
                                p.completed = p.completed.filter(
                                    taskId => taskId !== ev.taskId
                                );
                                p.subTasks = p.subTasks.filter(
                                    taskId => taskId !== ev.taskId
                                );
