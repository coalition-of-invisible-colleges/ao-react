...
export default function AoCardTabs(props: Props) {
  ...
  const renderedTabs = props.tabs.map((cardTab, i) => {
    ...
    return <AoCardTab id={cardTab.id} icon={cardTab.icon} tooltip={cardTab.tooltip} content={cardTab.content} onDrop={cardTab.onDrop} isSelected={isSelected} dropHoverMessage={cardTab.dropHoverMessage} onClick={() => toggleTab(tab)} key={i} />
  })
