    ...
    computed: {
      topcard() {
        ...
        let subpriorities = news[ndex].priorities.filter(tId => {
          ...
          return (
            subpriority.claimed.indexOf(this.$store.getters.member.memberId) ===
            -1
          );
        });
        ...
        return this.$store.getters.hashMap[
          subpriorities[subpriorities.length - 1]
        ];
      }
    }
  };
